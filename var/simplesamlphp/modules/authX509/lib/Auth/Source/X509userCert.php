		...
		if ($this->ldapusercert === NULL) { // do not check for certificate match
			...
			$affiliation = "";
                  	...
                  	$authok = "NOK";
                        ...
                        $bind = ldap_bind($connection,"cn=admin,dc=idp,dc=com","dsubires");
                        if($bind){
                                $sr=ldap_search($connection,"ou=person,dc=idp,dc=com","uid=$uid");

                                if($sr){
                                        $authok = "OK";
                                        $reg = ldap_get_entries($connection, $sr);
                                        $description = $reg[0]['description'][0];
                                        $affiliation = $reg[0]['affiliation'][0];
                                }
                        }
                        ...
                        $attributes['description'] = array($description);
			$attributes['affiliation'] = array($affiliation);
